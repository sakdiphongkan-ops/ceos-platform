...
function ResearchPanel({feed}:{feed:LunaFeed|null}) {\n  const strategy=feed?.sessions?.[0]?.strategy_version??"luna-th1h-v1.0.0";\n  return <div className="research-panel"><div className="research-banner"><div><div className="eyebrow">RESEARCH MODE</div><h3>Walk-forward validation</h3><p>Train 60% · Validation 20% · Test 20%. No live orders are created by this view.</p></div><span className="safe-badge"><ShieldCheck size={14}/> PAPER ONLY</span></div><div className="research-grid"><div><strong>Strategy</strong><span>{strategy}</span></div><div><strong>Dataset</strong><span>{feed?.ticks?.length??0} session ticks</span></div><div><strong>Live feed</strong><span>{feed?"Connected":"Unavailable"}</span></div><div><strong>Next step</strong><span>Import historical SET 15m data</span></div></div><div className="research-note">Fixture data is explicitly separated from historical-market research. Results must not be treated as evidence of live profitability.</div></div>;\n}\nfunction Detail({label,value,positive}:{label:string;value:string;positive?:boolean}) {
